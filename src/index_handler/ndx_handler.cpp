...
IndexID IndexHandle::lower_bound(const uint8_t *target) const {
    ...
    int key_idx = node.lower_bound(target);
    IndexID iid(node.pages->id.page_no, key_idx);
    return iid;
}
...
void IndexHandle::insert_entry(const u_int8_t *key, const RecordID &rid)
{
    ...
    // Insert the new key and record ID into the leaf node.
    node.insert_key(iid.slot_number, key);
    node.insert_record_id(iid.slot_number, rid);

    // This is now implicitly handled by the split logic, but a dedicated function
    // is better if the last key changes without a split.
    maintain_parent(node);

    // Handle node overflow by splitting, propagating splits up the tree if necessary.
    while (node.header->num_key > this->index_header.btree_order) {
        // Allocate a new sibling node.
        IndexNodeHandle brother_node = create_node();
        *brother_node.header = IndexPageHeader(NDX_NO_PAGE, node.header->parent, 0, 0,
                                             node.header->is_leaf, NDX_NO_PAGE, NDX_NO_PAGE);

        int parent_page_no = node.header->parent;

        if (parent_page_no == NDX_NO_PAGE) {
            IndexNodeHandle root = create_node();
            parent_page_no = root.pages->id.page_no;
            *root.header = IndexPageHeader(NDX_NO_PAGE, NDX_NO_PAGE, 0, 0, false, NDX_NO_PAGE, NDX_NO_PAGE);
            ...
            root.insert_record_id(0, curr_rid); // The first child is the old root.
            
            // Update parent pointers and the main index header.
            node.header->parent = parent_page_no;
            brother_node.header->parent = parent_page_no;
            this->index_header.root_page = parent_page_no;
        }
        
        IndexNodeHandle parent = get_node(parent_page_no);
        parent.pages->mark_dirty();
        
        uint8_t *popup_key;
        int split_idx;

        // FIX: Differentiate between leaf and internal node splits.
        if (node.header->is_leaf) {
            // --- LEAF SPLIT ---
            split_idx = (node.header->num_key + 1) / 2;
            popup_key = node.get_key(split_idx); // Key to be COPIED up.

            // Transfer keys/records to the new sibling.
            ...
            if (node.header->next_leaf != NDX_NO_PAGE) {
                IndexNodeHandle next_node = get_node(node.header->next_leaf);
                next_node.pages->mark_dirty();
                next_node.header->prev_leaf = brother_node.pages->id.page_no;
            }
            node.header->next_leaf = brother_node.pages->id.page_no;
            brother_node.header->prev_leaf = node.pages->id.page_no;

            if (this->index_header.last_leaf == node.pages->id.page_no) {
                this->index_header.last_leaf = brother_node.pages->id.page_no;
            }
        } else {
            split_idx = node.header->num_key / 2;
            popup_key = node.get_key(split_idx); // Key to be MOVED up.

            // Transfer keys/children to the new sibling.
            int num_transfer = node.header->num_key - split_idx - 1;
            brother_node.insert_keys(0, node.get_key(split_idx + 1), num_transfer);
            brother_node.insert_record_ids(0, node.get_record_id(split_idx + 1), node.header->num_child - (split_idx + 1));
            node.header->num_key = split_idx;
            node.header->num_child = split_idx + 1;

            // Update parent pointers of moved children.
            for (int i = 0; i < brother_node.header->num_child; ++i) {
                maintain_child(brother_node, i);
            }
        }

        // Insert the promoted key and pointer to the new sibling into the parent.
        int child_idx = parent.find_child(node);
        parent.insert_key(child_idx, popup_key);
        RecordID brother_rid(brother_node.pages->id.page_no, -1);
