...
static void check_pages(const std::list<PageID> &busy_page_ids) {
    EXPECT_EQ(PF_Manager::pager.free_list().size(), NUM_CACHE_PAGES - busy_page_ids.size());
    EXPECT_EQ(PF_Manager::pager.busy_list().size(), busy_page_ids.size());
    auto busy_it = PF_Manager::pager.busy_list().begin();
    for (const auto &pid : busy_page_ids) {
        EXPECT_EQ(pid, (*busy_it)->id);
        EXPECT_TRUE(PF_Manager::pager.in_cache(pid));
        EXPECT_EQ(PF_Manager::pager.busy_map().at(pid), busy_it);
        busy_it++;
    }
}
