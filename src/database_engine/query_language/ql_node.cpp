...
std::vector<Column_Metadata>::const_iterator QlNode::get_col(const std::vector<Column_Metadata> &rec_cols, const TabCol &target) {
    auto pos = std::find_if(rec_cols.begin(), rec_cols.end(), [&](const Column_Metadata &col) {
...
std::map<TabCol, Value> QlNode::rec2dict(const std::vector<Column_Metadata> &cols, const RM_Record *rec) {
    ...
    for (auto &col : cols) {
        ...
        if (col.type == TYPE_INT) {
            ...
            memcpy(&datetime_val, val_buf, sizeof(DateTime));
...
std::unique_ptr<RM_Record> QlNodeProj::rec() const {
    assert(!is_end());
    auto &prev_cols = _prev->cols();
    auto prev_rec = _prev->rec();
    auto &proj_cols = _cols;
    auto proj_rec = std::make_unique<RM_Record>(_len);
...
QlNodeTable::QlNodeTable(std::string tab_name, std::vector<Condition> conds) {
    ...
    Table_Metadata &tab = SM_Manager::db.get_table(_tab_name);
    _fh = SM_Manager::fhs.at(_tab_name).get();
    _cols = tab.cols;
...
void QlNodeTable::begin() {
    ...
    if (index_no == -1) {
        ...
        _scan = std::make_unique<RM_Iterator>(_fh);
    } else {
        // index is available, scan index
        auto ih = SM_Manager::ihs.at(IndexManager::get_index_name(_tab_name, index_no)).get();
        IndexID lower = ih->leaf_begin();
        IndexID upper = ih->leaf_end();
        ...
        _scan = std::make_unique<IndexIterator>(ih, lower, upper);
    }
    // Get the first record
    while (!_scan->is_end()) {
        _rid = _scan->get_RecordID();
...
bool QlNodeTable::eval_cond(const std::vector<Column_Metadata> &rec_cols, const Condition &cond, const RM_Record *rec) {
    ...
    int cmp = compare_keys(lhs, rhs, rhs_type, lhs_col->len);
    if (cond.op == OP_EQ) {
        return cmp == 0;
...
bool QlNodeTable::eval_conds(const std::vector<Column_Metadata> &rec_cols, const std::vector<Condition> &conds,
                             const RM_Record *rec) {
...
std::unique_ptr<RM_Record> QlNodeJoin::rec() const {
    assert(!is_end());
    auto record = std::make_unique<RM_Record>(_len);
