...
TEST(PF_ManagerTest, basic) {
    std::string path1 = "a.txt";
    if (PF_Manager::is_file(path1)) {
        PF_Manager::destroy_file(path1);
    }
    std::string path2 = "b.txt";
    if (PF_Manager::is_file(path2)) {
        PF_Manager::destroy_file(path2);
    }

    // create file
    PF_Manager::create_file(path1);
    EXPECT_THROW(PF_Manager::create_file(path1), FileExistsError);

    // is file
    EXPECT_TRUE(PF_Manager::is_file(path1));
    EXPECT_FALSE(PF_Manager::is_file(path2));

    // open file
    int fd = PF_Manager::open_file(path1);
    EXPECT_THROW(PF_Manager::open_file(path2), FileNotFoundError);
    EXPECT_THROW(PF_Manager::open_file(path1), FileNotClosedError);

    // close file
    EXPECT_THROW(PF_Manager::close_file(fd + 1), FileNotOpenError);
    PF_Manager::close_file(fd);
    EXPECT_THROW(PF_Manager::close_file(fd), FileNotOpenError);

    // destroy file
    EXPECT_THROW(PF_Manager::destroy_file(path2), FileNotFoundError);
    fd = PF_Manager::open_file(path1);
    EXPECT_THROW(PF_Manager::destroy_file(path1), FileNotClosedError);
    PF_Manager::close_file(fd);
    PF_Manager::destroy_file(path1);
    EXPECT_THROW(PF_Manager::destroy_file(path1), FileNotFoundError);
}

static void check_pages(const std::list<PageID> &busy_page_ids) {
    EXPECT_EQ(PF_Manager::pager.get_free_pages().size(), NUM_CACHE_PAGES - busy_page_ids.size());
    EXPECT_EQ(PF_Manager::pager.get_busy_pages().size(), busy_page_ids.size());
    auto busy_it = PF_Manager::pager.get_busy_pages().begin();
    for (const auto &pid : busy_page_ids) {
        EXPECT_EQ(pid, (*busy_it)->id);
        EXPECT_TRUE(PF_Manager::pager.in_cache(pid));
        EXPECT_EQ(PF_Manager::pager.get_busy_map().at(pid), busy_it);
        busy_it++;
    }
}

TEST(PF_PagerTest, lru) {
    srand((unsigned)time(nullptr));

    std::vector<std::string> paths{"0.txt", "1.txt", "2.txt", "3.txt"};
    std::vector<int> fds;
    for (const auto &path : paths) {
        if (PF_Manager::is_file(path)) {
            PF_Manager::destroy_file(path);
        }
        PF_Manager::create_file(path);
        int fd = PF_Manager::open_file(path);
        fds.emplace_back(fd);
    }
    ...
    for (int i = 0; i < NUM_CACHE_PAGES; i++) {
        ...
        for (int fd : fds) {
            PF_Manager::pager.create_page(fd, i);
    ...
    while (busy_it != busy_page_ids.end()) {
        ...
        PF_Manager::pager.fetch_page(busy_it->fd, busy_it->page_no);
...
class MockPager {
  public:
    ...
    void check_disk(int fd, int page_no) {
        EXPECT_TRUE(!PF_Manager::pager.in_cache({fd, page_no}));
        uint8_t buf[PAGE_SIZE];
        PF_Pager::read_page(fd, page_no, buf, PAGE_SIZE);
...
TEST(PF_PagerTest, readwrite) {
    ...
    for (int i = 0; i < 100000; i++) {
        ...
        if (!is_created[page_id]) {
            // create page
            Page *page = PF_Manager::pager.create_page(fd, page_no);
        ...
        // flush
        if (rand() % 20 == 0) {
            PF_Manager::pager.flush_page(page);
            mock.check_disk(fd, page_no);
        }
        // flush the entire file
        if (rand() % 200 == 0) {
            PF_Manager::pager.flush_file(fd);
        }
        // re-open file
        if (rand() % 200 == 0) {
            PF_Manager::close_file(fd);
            int new_fd = PF_Manager::open_file(filenames[file_idx]);
            if (new_fd != fd) {
                fds[file_idx] = new_fd;
