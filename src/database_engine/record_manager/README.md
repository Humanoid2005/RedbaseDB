...
┌─────────────────────────────────────────────────────────────────┐
│                      RM_Manager                                  │
│              (Static Record File Manager)                        │
├─────────────────────────────────────────────────────────────────┤
│  • create_file(filename, record_size)                            │
│  • destroy_file(filename)                                        │
│  • open_file(filename) → RM_FileHandle                           │
│  • close_file(RM_FileHandle)                                     │
└──────────────────────────┬──────────────────────────────────────┘
                           │ creates/manages
                           ↓
┌─────────────────────────────────────────────────────────────────┐
...
├─────────────────────────────────────────────────────────────────┤
│  Public API:                                                     │
│  • insert_record(data) → RecordID                                │
│  • delete_record(RecordID)                                       │
│  • update_record(RecordID, new_data)                             │
│  • get_record(RecordID) → unique_ptr<RM_Record>                  │
│  • force_pages()  - Flush dirty pages                            │
│                                                                   │
│  Internal State:                                                 │
│  • RM_FileHeader hdr  - File metadata                            │
│  • PF_Pager* pager    - Page file handler                        │
└──────────────────────────┬──────────────────────────────────────┘
                           │ uses
                           ↓
┌─────────────────────────────────────────────────────────────────┐
│              Record File Structure (Disk-Based)                  │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  Page 0: File Header                                             │
│  ┌──────────────────────────────────────┐                       │
...
│  ┌────────────────────────────────────────────────┐             │
│  │ RM_PageHeader                                   │             │
│  │ • int next_free (linked free list)              │             │
│  │ • int num_records (occupied slots)              │             │
│  ├────────────────────────────────────────────────┤             │
│  │ Bitmap (slot allocation)                        │             │
│  │ [1 bit per slot: 0=free, 1=occupied]            │             │
│  │                                                 │             │
│  │ Example (8 slots):                               │             │
│  │ Bitmap: 10110100                                 │             │
│  │         ^^^^^^^^                                 │             │
│  │         slots:  0=occupied, 1=free, 2=occupied, │             │
│  │                 3=occupied, 4=free, ...          │             │
│  ├────────────────────────────────────────────────┤             │
│  │ Record Slots (Fixed Size)                       │             │
│  │ ┌──────────┐                                    │             │
...
│                                                                   │
│  Free Page List:                                                 │
│  first_free → Page 5 → Page 12 → Page 23 → NULL                 │
│  (Linked via next_free field)                                    │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           ↓
┌─────────────────────────────────────────────────────────────────┐
...
├─────────────────────────────────────────────────────────────────┤
│  • get_record(slot_no) → uint8_t*                                │
│  • insert_record(data) → slot_no                                 │
│  • delete_record(slot_no)                                        │
│  • get_header() → RM_PageHeader                                  │
│  • get_bitmap() → Bitmap                                         │
│  • is_full() / has_free_slot()                                   │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           ↓
┌─────────────────────────────────────────────────────────────────┐
...
│  • is_end()        - Check if scan complete                      │
│  • get_RecordID()  - Get current (page_no, slot_no)              │
│                                                                   │
│  Algorithm:                                                      │
│    FOR page = 1 TO num_pages:                                    │
│        FOR slot = 0 TO num_records_per_page:                     │
│            IF bitmap[slot] == occupied:                          │
│                YIELD RecordID(page, slot)                        │
└──────────────────────────┬──────────────────────────────────────┘
                           │ both use
                           ↓
┌─────────────────────────────────────────────────────────────────┐
...
insert_record(data)
    ↓
RM_FileHandle::insert_record()
    ↓ find/allocate free page
RM_FileHandle::get_free_page()
    ↓ insert into page
RM_PageHandle::insert_record()
    ↓ mark slot in bitmap
Bitmap::set(slot_no)
    ↓ write to page buffer
PF_Pager::mark_dirty(page_no)
    ↓ eventually flush
Disk I/O (.rdb file)
```
