...
struct ShowDatabases : public TreeNode {};

struct CreateDatabase : public TreeNode {
    std::string db_name;

    CreateDatabase(std::string db_name_) : db_name(std::move(db_name_)) {}
};

struct DropDatabase : public TreeNode {
    std::string db_name;

    DropDatabase(std::string db_name_) : db_name(std::move(db_name_)) {}
};

struct UseDatabase : public TreeNode {
    std::string db_name;

    UseDatabase(std::string db_name_) : db_name(std::move(db_name_)) {}
};
...
struct DateTimeLit : public Value {
    DateTime val;  // Store as DateTime object

    DateTimeLit(DateTime val_) : val(val_) {}
    
    // Constructor from string for parser convenience
    DateTimeLit(const std::string& str_val) {
        // Parse "YYYY-MM-DD HH:MM:SS" format
        int year, month, day, hour, minute, second;
        sscanf(str_val.c_str(), "%d-%d-%d %d:%d:%d", 
