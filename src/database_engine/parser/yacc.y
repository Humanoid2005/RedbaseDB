...
// Use custom YYSTYPE from ast.h (which is ast::SemValue)
...
// keywords
%token SHOW TABLES DATABASES CREATE TABLE DATABASE DROP USE DESC INSERT INTO VALUES DELETE FROM
WHERE UPDATE SET SELECT INT CHAR FLOAT DATETIME INDEX AND EXIT HELP
// non-keywords
...
dbStmt:
    ...
    {
        $$ = std::make_shared<DropDatabase>($3);
    }
    |   USE tbName
    {
