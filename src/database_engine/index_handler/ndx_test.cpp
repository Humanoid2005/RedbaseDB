...
public:
    ...
    int debug_count = 0;
        while (!scan.is_end() && it != mock.end()) {
            ...
            if (debug_count < 40) {
                auto curr = scan.current_index_id();
                std::cout << "iter key=" << it->first << " page=" << curr.page_number << " slot=" << curr.slot_number
                          << " mock_rid=" << mock_rid.page_no << "," << mock_rid.slot_no
                          << " tree_rid=" << rid.page_no << "," << rid.slot_no << std::endl;
            }
